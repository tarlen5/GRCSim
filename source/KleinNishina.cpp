...
*/
{
  ...
  Vec3D v =
      (e - en * n) / root_e; //! add random tiny EPS direction when root_e=0
  ...
  Vec3D e1 =
      ((gamma * a_ + gammabeta) * n + b_ * v + sin_theta * sin_phi * (n ^ v)) /
      c;
...
*/
{
  ...
  return 3.0 / 4.0 *
         (Ln1 - 4.0 * Ln2 - 8.0 * Ln3 + z / x * (1.0 + arg) / 2.0 / arg / arg);
}
...
*/
{
  ...
  /*
    ...
    {
    std::cout << "IS[j]/IS[m_num_int-1] = " << IS[j]/IS[m_num_int-1] <<
    std::endl;
    }
  */
  ...
  */
  ...
  std::cout << std::endl
            << std::endl
            << "chi_2 = " << chi_2 << std::endl
            << "IntegralRatio = " << IntegralRatio << std::endl
            << "Energy of Photon = " << x << " kT." << std::endl;
...
VEC3D_T KleinNishina::PropagationLengthBB(VEC3D_T Ebb, Vec4D &P_lep, Vec4D &P_p)
...

{
  ...
  if (fabs(1.0 - m_egy_bb / Ebb) > m_DE) {
    ...
    m_egy_dens_bb = q * m_PI2d6 * m_PI2d6 * 8.0 * 3.0 / 5.0 *
                    Ebb; // Photon energy density [eV cm^-3]
    m_egy_bb = Ebb;
...
*/
{
  ...
  while (epsilon < egy_max) {
    ...
    if (epsilon < CMB_max)
      temp_dirbr += ebl_model->GetCMBR(m_lambda_vec[Nstep]);
...
*/
{
  ...
  while (fabs(y - y_) > m_DE * fabs(y)) {
    ...
    if (y <= b)
      UI = m_IRd_b * (f1 - m_IRd_f1c) / z - (f2 - m_IRd_f2c) / z / z;
    else {
...
*/
{
  ...
  f2 = 3.0 / 4.0 *
       (arg * Ln1 - x * (1.0 + arg) / arg / 2.0 + 8.0 * Ln1 + 8.0 * Ln2 -
        4.0 * PLg);
