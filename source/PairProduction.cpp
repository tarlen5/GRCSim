  ...
  */
   ...
   */
  {
    ...
    while ( tau < tauFinal ) {  // Also exits when z < z_min (below)
      ...
      double z3=z1*z2;
      double z4=z1*z3;
      Q=sqrt(m_OmegaR_d*z4+m_OmegaM_d*z3+m_OmegaL_d+(1.0-m_Omega0_d)*z2);
...
*/
  {
	...
	std::cout<<std::endl<<"In between m_q_vec[i] bins"<<
	  "\t m_q_vec[i] before adjustment = "<<m_q_vec[i]<<"\tchi_rand = " <<
	  chi_rand<<std::endl;
	m_q_vec[i] = (m_q_vec[i+1] + m_q_vec[i])/2.0;
	std::cout<<"m_q_vec[i] after adjustment = "<<m_q_vec[i]<<
	  std::endl<<std::endl;
      } else {
  ...
  */
  {
    ...
    ////////////////////////////////////////////////////////////////
    VEC3D_T q =PhysConst::eV_MELEC*PhysConst::eV_MELEC/bg_ph_p4.r0/gam_ph_p4.r0;
    if (q > D1) {
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  VEC3D_T PairProduction::ImpactAngle(VEC3D_T q)
      ...
      */
    {
      ...
      if ( q < 0.001 ) etaminus = q/D2/(D1+phi);
      else             etaminus = (D1 - phi)/D2;
      ...
      VEC3D_T G=(g1*log(z)-g2+PolyLog1(zinv)-PolyLog1(z)+log(zinv)*log(q/D4));
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  */
 {
   ...
   if (Ecm < PhysConst::eV_MELEC){
     ...
     std::cerr<<"Photon Energy - mass electron = "<<Ecm - PhysConst::eV_MELEC<<" eV";
     ...
     /////////////////////////////////////////////////////
     VEC3D_T s_before=(gam_ph_p4+bg_ph_p4)*
       (gam_ph_p4+bg_ph_p4);
     VEC3D_T s_after =(elec_p4+pos_p4)*
       (elec_p4+pos_p4);
     VEC3D_T eps = 1.E-12;
      ...
      */
  {
    ...
    while (fabs(eps) > m_DE_int) {
      ...
      if ( y < D0 ) j1 = D1 - lambda*y;
      else          j1 = ((D1-y)*(D1+y)+ x*y*y)/(D1+lambda*y);
