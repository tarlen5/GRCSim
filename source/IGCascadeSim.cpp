...
void IGCascadeSim::InitializeEBL(const string &ebl_model_file) {
  ...
  if (ebl_model_file.find("table") == string::npos) {
    ...
    // Initialize DIRBR:
    DIRBR_ERR ebl_err =
        m_ebl->SetDIRBR(lambda_vec.size(), &lambda_vec[0], &nuFnu_vec[0]);
...
void IGCascadeSim::RunCascade(const int numIterations) {
  ...
  std::cout << "\nSimulation took: " << (int(seconds) / 3600) << " hr, "
            << (int(seconds) % 3600) / 60 << " min, " << int(seconds) % 60
            << " sec." << endl;
...
void IGCascadeSim::RunSinglePhotonCascade(void) {
  ...
  while (!lepton_stack.empty()) {
    ...
    bool pair_prod = PropagateSecondaryPhoton(GammaPhoton, Electron, Positron);
...
void IGCascadeSim::CreateDirectGamma(RelParticle &GammaPhoton) {
  ...
  VEC3D_T E_e =
      m_egy_cascade * PhysConst::TeV * (1.0 + m_ze); // energy at EMISSION
  Vec4D R4_e(0., 0., 0., 0.);
  Vec3D n1(0., 0., 1.);
...
double IGCascadeSim::GetOpticalDepthVal(double egy, double z) {
  double TeV = 1.0e12;
...
void IGCascadeSim::PropagatePhotonToObserver(RelParticle &GammaPhoton) {
...
void IGCascadeSim::SaveDirectPhoton(RelParticle &GammaPhoton,
  ...
  if (GammaPhoton.m_z_s > 1.0e-15) { // Should never happen
    ...
    cerr << "z_s: " << GammaPhoton.m_z_s << " egy: " << GammaPhoton.m_p4.r0
         << endl;
  }
}
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
void IGCascadeSim::SaveToLowEnergyFile(RelParticle &Particle) {
  ...
  low_egy_stream << Particle.m_p4.r0 << " " << Particle.m_z << " "
                 << Particle.m_q << endl;
...
void IGCascadeSim::SaveLepton(RelParticle *Lepton) {
  ...
  // ofile<<Lepton->m_tag<<" "<<Lepton->m_q<<" "<<Lepton->m_z<<"
  // "<<Lepton->m_p4.r0<<" "<<Lepton->m_p4.r<<" "<<Lepton->m_r4.r0<<"
  // "<<Lepton->m_r4.r<<endl; ofile.close();
  ...
  LeptonData ld = {Lepton->m_tz,  Lepton->m_tegy, Lepton->m_tpx,
...
void IGCascadeSim::SaveToTrackTimeDelayFile(RelParticle &Photon) {
  ...
  ofile << std::fixed << Photon.m_p4.r0 << " " << theta << " " << phi << " "
        << theta_p << " " << xi << " " << Photon.m_r4.r0 << " "
        << Photon.m_elec_time << std::endl;
