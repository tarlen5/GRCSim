...
EBLModelsFile::~EBLModelsFile() {
  ...
  if (fWritable) {
    ...
    if ((fwrite(&fEBLNumModels, sizeof(fEBLNumModels), 1, fFP) != 1) ||
        ...
        (fwrite(&fAcceptableModelsNum, sizeof(fAcceptableModelsNum), 1, fFP) !=
         1)) {
      perror("fwrite [0]");
      exit(EXIT_FAILURE);
    }
...
EBLModelsFile::EBLModelsFile(
    ...
    : fFileName(filename), fFP(), fWritable(true), fSeekPos(0),
      ...
      fEBLNumModels(0), fAGNNumParameters(num_agn_param), fAGNParameterNames(),
      fAGNModelsLookup(), fAGNModels(), fAGNModelsUnwritten(), fAGNNumModels(0),
      ...
      fVHEDatasetName(vhe_dataset_name), fVHENumData(vhe_num_data), fVHEDataE(),
      fVHEDataI(), fVHEDataIError() {
  // **************************************************************************
  ...
  // --------------------------------------------------------------------------
  if (!b_write(fVHENumData, fFP) || !b_write(fVHEDataE, fFP) ||
      !b_write(fVHEDataI, fFP) || !b_write(fVHEDataIError, fFP)) {
    perror("write [4]");
    exit(EXIT_FAILURE);
  }
  ...
  // --------------------------------------------------------------------------
  if (!b_write(fEBLNumParameters, fFP) || !b_write(fAGNNumParameters, fFP) ||
  ...
  fSeekPos = ftell(fFP);
  if (!b_write(fEBLNumModels, fFP) || !b_write(fAGNNumModels, fFP) ||
...
void EBLModelsFile::writeEBLModel() {
  ...
  if (!b_write(fEBLParameters, fEBLNumParameters, fFP) ||
      !b_write(fEBLTau01, fFP) || !b_write(fEBLTau1, fFP) ||
      !b_write(fEBLTau10, fFP) || !b_write(fEBLIStars, fFP) ||
...
EBLModelsFile::EBLModelsFile(const std::string &filename)
    : fFileName(filename), fFP(), fWritable(false), fSeekPos(0),
      fEBLNumParameters(0), fEBLLambda(), fEBLParameters(0), fEBLTau01(),
      fEBLTau1(), fEBLTau10(), fEBLIStars(), fEBLIDust(), fEBLNumModels(0),
      fAGNNumParameters(), fAGNParameterNames(), fAGNModelsLookup(),
      fAGNModels(), fAGNModelsUnwritten(), fAGNNumModels(0),
      fAcceptableModels(), fNextAcceptableModel(), fVHEDatasetName(),
      fVHENumData(), fVHEDataE(), fVHEDataI(), fVHEDataIError() {
  // **************************************************************************
  ...
  // --------------------------------------------------------------------------
  if (!b_read(fVHENumData, fFP) || !b_read(fVHEDataE, fFP) ||
      !b_read(fVHEDataI, fFP) || !b_read(fVHEDataIError, fFP)) {
    perror("read [4]");
    exit(EXIT_FAILURE);
  }
  ...
  // --------------------------------------------------------------------------
  if (!b_read(fEBLNumParameters, fFP) || !b_read(fAGNNumParameters, fFP) ||
  ...
  if (!b_read(fEBLNumModels, fFP) || !b_read(fAGNNumModels, fFP) ||
...
bool EBLModelsFile::readNextEBLModel() {
  ...
  if (!b_read(fEBLParameters, fEBLNumParameters, fFP) ||
      !b_read(fEBLTau01, fFP) || !b_read(fEBLTau1, fFP) ||
      !b_read(fEBLTau10, fFP) || !b_read(fEBLIStars, fFP) ||
