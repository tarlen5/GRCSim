...
double Table1D::LinInterpolate(const double &xval) {
  ...
  if (xval < m_xvector[0]) {
    std::cerr << "\nERROR: " << __PRETTY_FUNCTION__ << "\nxval cannot be "
              << "less than (or equal to) the first table value." << std::endl
              << std::endl;
    exit(EXIT_FAILURE);
  }
    ...
    if (irow == 1) {
      ...
      if (m_col != icol) {
        std::cerr << "\nERROR: in " << __PRETTY_FUNCTION__
                  << " incorrect format" << " of file: " << tablefile
...
Table2D::Table2D(std::vector<double> &rows_vec, std::vector<double> &cols_vec) {
...
*/
{
  ...
  double val = (1 - interp_t) * (1 - interp_u) * y1 +
               interp_t * (1 - interp_u) * y2 + interp_t * interp_u * y3 +
               (1 - interp_t) * interp_u * y4;
