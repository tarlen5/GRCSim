...
void DIRBR_Redshift::InitializeIntegral(void) {
  ...
  for (int i = 1; i < m_nsteps; i++) {
    ...
    farray[i] = 1.0 / y *
                ((1.0 + y - y * y / 2.0) * log((1 + factor) / (1 - factor)) -
                 (1.0 + y) * factor);
    m_qarray[i] = y;
  }
  ...
  for (int i = 1; i < m_nsteps; i++) {
    m_Farray[i] =
        2.0 * m_qarray[i] * m_qarray[i] * m_qarray[i] * farray[i] * m_dlnq;
...
*/
{
  ...
  /////////////////////////////////////////////////////////////////
  if ((z < m_zmin) || (z > m_zmax) || (lambda < m_lambda_min) ||
      (lambda > m_lambda_max)) {
    std::cerr << "ERROR: ";
...
*/
{
  ...
  double opt_depth_scale = 3.0 / 2.0 * M_PI * sigma_T * (E * TeV) / melec_ev /
                           melec_ev / Ho / eV_to_nJ;
  ...
  //       if ( (z1*lambda) < m_lambda_min || (z1*lambda) > m_lambda_max)
  //       continue;
  ...
  for (z = 0.0; z < RedShift; z += m_dz) {
    ...
    if (m_dz < (RedShift - z))
      OpticalDepth += tau * m_dz;
    else
      OpticalDepth += tau * (RedShift - z);
  }
...
double DIRBR_Redshift::Distance(double &z) { return 0.0; }
